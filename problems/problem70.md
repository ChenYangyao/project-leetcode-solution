...
We try to solve this problem mathematically. First, we consider the possible number of combinations for `k` twos and `n-2k` ones. It is easy to show that the answer is
![equation](https://latex.codecogs.com/gif.latex?%7B%7Bn-k%7D%5Cchoose%7Bn-2k%7D%7D).
(Imaging we need to put `k` identical balls into `n-2k+1` different boxes)
