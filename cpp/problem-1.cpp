 ...
 *
 * 
 * The length of candidate queue never exceeds O(m),  so the search of real shoetest
 * takes O(m) time, if implemented like the following way.
 * Total time : O(mn). Space: O(n)
 *
 * If m is considerable large than log(n), you may replace the linear search with a 
 * balanced tree (e.g., RB, AVL, ...). Then the total time cost is O(n*log(n)).
 */
